...
const Category = () => {
    ...
    */
    ...
    const categoryList2 = (Category2.reduce((a,v)=>{
            ...
            if(categoryList.length === 0 || isTrue){
              ...
              // }
                for(let i=0;i<categoryList2.length;i++){
                  for(let j=0;j<v.category2.length;j++){
                      if(categoryList2[i] === v.category2[j]){
                          isTrue = true;
                          j = v.category2.length;
                      }
                      if(j === v.category2.length-1){
                          isTrue = false;
                          i = categoryList2.length + 1;
                      }
                  }
                }
    ...
    return (
        ...
        <HeaderComponent/>
            ...
            <div className={testStyle.test} style={{marginTop:"51px", borderTop:"1px solid #000"}}>
                ...
                <h2 style={{fontWeight:"bold"}}>상세 카테고리</h2>
                <ul>
                    {Category2.map(({id,name,isCheck})=>(<li key={id} className={isCheck ? "checked" : ""} onClick={()=>{
                        setCategory2(Category2.map(ele => {
                            ele.isCheck = false;
                            if(ele.id === id){
                                ele.isCheck = !ele.isCheck
                            }
